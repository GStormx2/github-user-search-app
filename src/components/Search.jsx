...
const Search = () => {
    ...
    return (
        <div className="flex justify-between items-center bg-app-card-light p-2 mb-7 rounded-lg dark:shadow-dark-dreamy shadow-dreamy dark:bg-app-card-dark">
            <div className="flex items-center gap-7">
                <img className="pl-3" src={search_logo} alt="search logo"></img>
                <input className="text-search focus:placeholder-opacity-0 dark:text-white dark:bg-app-card-dark w-72 focus:outline-none placeholder-search dark:placeholder-white" ref={usernameRef} placeholder="Search Github Username..."></input>
                
            </div>
            <div className="flex items-center">
                <p className="text-red-600 font-bold text-h3 mr-5">{status === 'No Result' ? status : ''}</p>
                <div onClick={handleSearchClick} className="bg-app-blue py-3 px-5 text-h3 font-medium text-white rounded-lg hover:cursor-pointer hover:bg-app-lightblue">Search</div>
            </div>
        </div>
    );
}
