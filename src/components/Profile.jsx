...
const Profile = () => {
    ...
    if (status === 'init') {
        ...
        return <div className="bg-app-card-light transition-all duration-300 dark:bg-app-card-dark p-10 shadow-dreamy rounded-lg font-medium dark:text-white dark:shadow-dark-dreamy">Woops! :( Try another one</div>
    }
        
    else {
        return (
            <div className="flex transition-all duration-300 bg-app-card-light dark:bg-app-card-dark p-10 shadow-dreamy rounded-lg dark:shadow-dark-dreamy">
                <div className="w-1/4">
                ...
                <div className="flex-1">
                    <div className="flex justify-between items-center">
                        <h1 className="text-h1 font-bold leading-h1 transition-all duration-300 dark:text-white">{userData.name}</h1>
                        <h1 className="text-body leading-body text-app-gray transition-all duration-300 dark:text-white">{userData.joined_date}</h1>
                    </div>
                    <p className="text-h3 leading-h3 text-app-blue mb-7">{`@${userData.username}`}</p>
                    <p className="text-body leading-body mb-7 text-app-gray transition-all duration-300 dark:text-white">{userData.bio === null ? 'User has no Bio.' : userData.bio}</p>
                    <div className="grid grid-cols-3 grid-rows-1 bg-app-light-new transition-all duration-300 dark:bg-app-dark py-5 px-10 rounded-lg mb-7 dark:text-white">
                         <div>
                             <p className="text-h4 leading-h4 text-app-blueish transition-all duration-300 dark:text-white">Repos</p>
                    ...
                    <div className="grid grid-cols-2 grid-rows-2 gap-4">
                         <div className="flex gap-3 items-center text-app-blueish">
                             ...
                             <p className={"" + (userData.location === null ? "text-gray-500" : "text-app-blueish transition-all duration-300 dark:text-white")}>{userData.location === null ? 'Not available' : userData.location}</p>
                         </div>
                         <div className="flex gap-3 items-center">
                             <img src={theme==='dark' ? twitter_logo_svg_light : twitter_logo_svg_dark} alt="twitter logo"></img>
                             <p className={"" + (userData.twitter === null ? "text-gray-500" : "text-app-blueish transition-all duration-300 dark:text-white")}>{userData.twitter === null ? 'Not Available' : userData.twitter}</p>
                         </div>
